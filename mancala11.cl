...
(defun comer (tablero lado pos)
  (if (equal pos 6)
    0
    (let ((fichas (get-fichas tablero lado pos)))
      (+ (comer tablero lado (+ 1 pos))
        (if (and (< (+ fichas pos) 6) 
          (> (+ fichas pos) 0)
          (equal (get-fichas tablero lado (+ fichas pos)) 0))
      -5
      2)))))

;; Cuenta las fichas que tiene el jugador (sin contar kalaha)
(defun agua-fuera (tablero contrario)
  (- (cuenta-fichas tablero contrario 0)
    (get-fichas tablero contrario 6)))
...
; (partida 0 2 (list *gazor3*      *nacho*))
; (partida 0 2 (list *nacho*      *gazor3*))

; (partida 0 2 (list *jdr-nmx-Regular*      *nacho*))
; (partida 0 2 (list *nacho*      *jdr-nmx-Regular*))
